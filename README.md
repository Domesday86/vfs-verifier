...
- `--log-level <level>` - Set console log level: `trace`, `debug`, `info` (default), `warn`, `error`, `critical`, `off`
- `--log-file <path>` - Write full debug logging to file (debug output always generated internally)

#### VFS Verification Options
...
- `input.bin` - Input binary data file from efm-decoder in data mode
...
### Input Requirements
- Binary data file from efm-decoder in data mode
...
```bash
# Unified decode with verification
efm-decoder --mode data --output-metadata input.efm final.bin
...
- efm-decoder data-mode parameter tuning
